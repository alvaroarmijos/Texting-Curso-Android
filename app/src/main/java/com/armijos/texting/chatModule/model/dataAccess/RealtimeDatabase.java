...
import com.armijos.texting.chatModule.model.LastConnectionEventListener;
import com.armijos.texting.chatModule.model.MessageEventListener;
import com.armijos.texting.chatModule.model.SendMessageListener;
...
public class RealtimeDatabase {
    ...
    public void subscribteToMessages(String myEmail, String friendEmail, MessageEventListener listener){
    ...
    public void onsubscribeToMessages(String myEmail, String friendEmail){
        if (mMessagesEventListener != null){
    ...
    public void subscribeToFriend(String uid, LastConnectionEventListener listener){
                    ...
                    try {
                        ...
                        if (lastConnectionWith != null && !lastConnectionWith.isEmpty()){
                            ...
                            if (values.length > 0){
                                ...
                                if (values.length > 1){
                                    uidConeectedFriend = values[1];
                                }
                            }
                        }
                    }
                    listener.onSuccess(lastConnectionFriend == Constants.ONLINE_VALUE,
                            lastConnectionFriend, uidConeectedFriend);
        ...
        //ofline
        mDatabaseAPI.getUserReferenceByUid(uid).child(User.LAST_CONECTION_WITH).keepSynced(true);
        mDatabaseAPI.getUserReferenceByUid(uid).child(User.LAST_CONECTION_WITH)
